 ...
 */
  ...
  */
 const getFilePath = filename => path.resolve(__dirname, '../../config/', `${filename}.json`)

 // getConfig :: FileName -> Config
 const getConfig = configFile => JSON.parse(fileSystem.readFileSync(getFilePath(configFile), 'utf-8'))

 // getSubPathOfConfig :: String -> Config -> String -> ObjectProperty
 const getSubPathOfConfig = R.curry((initialProp, configFile, secondaryProp) => R.path([initialProp, secondaryProp], getConfig(configFile)))
  ...
  */
 const readFilePromise = path => new Promise((resolve, reject) => fileSystem.readFile(path, 'utf8', (err, data) => {
       if (err) reject(err)
       resolve(data)
     }))
  ...
  */
 const getConfigPromise = R.pipe(getFilePath, readFilePromise, p => p.then(JSON.parse))

 const sendProperty = R.curry((res, propName, data) => res.send(R.prop(propName, data)))
 ...
 const sendPropertyFromConfig = (res, configName, propName) => getConfigPromise(configName)
