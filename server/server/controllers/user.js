...
import { head, toUpper, pathOr } from 'ramda'
...
const getFullName = ({ firstName, lastName }) => (firstName && lastName ? `${firstName} ${lastName}` : 'Hello')
const getInitials = ({ firstName, lastName }) => (firstName && lastName ? `${toUpper(head(firstName))}${toUpper(head(lastName))}` : '')
  ...
  Promise.all([
   ...
   .then(([routes, user]) => {
     if (!user) {
      ...
      return user.comparePassword(req.body.password, (passwordError, isMatch) => {
...
const SignJWT = id => jwt.sign({ _id: id }, config.jwtSecret, { expiresIn: '20m' })
...
function forgotPassword(req, res, next) {
  ...
  .then((foundUser) => {
    ...
    return User.findOneAndUpdate({ _id: foundUser._id }, { $set: { resetToken: SignJWT(foundUser._id) } }, { new: true, useFindAndModify: false })
  })
  .then((updatedUser) => {
    const mailOptions = {
...
function updateAddress(req, res, next) {
  ...
  return User.findOneAndUpdate({ _id: userId }, { addresses: [req.body] }, { new: true })
    .then((user) => {
      const returnObj = setReturnObject(user, req.session.userRoutes)
      return res.json(returnObj)
    })
